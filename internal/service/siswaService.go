...
func GetSiswaId(idSiswa string)(*model.SiswaList, error) {
	row := database.DbMain.QueryRow(`SELECT sw_id, sw_nama, sw_email, sw_namamandarin, sw_jenisidentitas, sw_noidentitas, sw_tempatlahir, sw_tanggallahir, sw_tempattinggal, sw_nokontak, sw_jenissiswa, sw_userupdate, sw_lastupdate FROM hd_siswa WHERE sw_id = ? AND sw_isdelete = "n"`,idSiswa)
...
func GetAllSiswa()([]model.SiswaList,error) {
	QueryData := `SELECT sw_id, sw_nama, sw_email, sw_namamandarin, sw_jenisidentitas, sw_noidentitas, sw_tempatlahir, sw_tanggallahir, sw_tempattinggal, sw_nokontak, sw_jenissiswa, sw_userupdate, sw_lastupdate FROM hd_siswa WHERE sw_isdelete = "n"`
...
func GenerateSiswaId() (string, error) {
	...
	QueryData := `SELECT MAX((REPLACE(hd_siswa.sw_id,?,'') * 1)) as total FROM hd_siswa WHERE sw_id LIKE ?`
	...
	if lastNumber != "" {
		finalNumber,_ = strconv.Atoi(lastNumber)
...
func UpdateSiswa(siswaData *model.SiswaList) (bool,error) {
	...
	if siswaData.NamaMandarin.Str != "" {
		if UpdateString != "" {
			UpdateString += `, `
		}
		UpdateString += `sw_namamandarin = ? `
		args = append(args,siswaData.NamaMandarin.Str)
	}
...
func InsertDataExcel(insertData []model.SiswaList)(bool, error) {
	...
	for i,record := range insertData {
		...
		InsertPlaceholder += "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
		InsertValues = append(InsertValues,record.SiswaId,record.NamaSiswa,record.EmailSiswa,record.NamaMandarin,record.JenisIdentitas,record.NoIdentitas,record.TempatLahir,record.TanggalLahir,record.TempatTinggal,record.NoKontak,record.NoKontak,record.PasswordSiswa,"n",record.JenisSiswa,record.UserUpdate,record.LastUpdate)
	}
	InsertQuery += InsertPlaceholder
	_, err := database.DbMain.Exec(InsertQuery)
	if err != nil {
